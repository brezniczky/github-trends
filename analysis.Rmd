---
title: "Picking A Language for Analytics and Algorithms (Working Draft)"
author: "Janca"
...
grab.columns = function(filename, names, transform) {
  frame = read.csv(sprintf("input/%s", filename))
  if (!missing(transform)) {
    if (length(transform) != 2) {
      stop("Invalid transform length.")
    }
    orig = transform[1]
    new  = transform[2]
    colnames(frame)[colnames(frame) == orig] = new
  }
...
counts = grab.columns("deep_learning_trends.csv", c("C++", "Java", "MatLab", "Python"),
                      transform=c("C..", "C++"))
```
...
```{r echo=FALSE}
counts = grab.columns("analysis_trends.csv", c("R", "Python", "Java"))
...
```{r echo=FALSE}
counts = grab.columns("spark_trends.csv", c("Python", "Java", "Scala"))
...
```{r echo=FALSE}
counts = grab.columns("ml_trends.csv", c("Python", "Java", "R"))
...
```{r echo=FALSE}
counts = grab.columns("big_data_trends.csv", c("Python", "Java", "R", "Scala"))
